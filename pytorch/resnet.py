...
def scs3x3(num_input_features: int,  num_output_features: int, stride: int = 1, groups: int =1, dilation: int = 1) -> SharpCosSim2d:
    return SharpCosSim2d(num_input_features,
                                        num_output_features,
                                        kernel_size=3,
                                        stride = stride,
                                        padding = dilation,
...
def scs1x1(num_input_features: int,  num_output_features: int, stride: int = 1) -> SharpCosSim2d:
    return SharpCosSim2d(num_input_features,
                                        num_output_features,
                                        kernel_size=1,
                                        stride = stride)
...
class ResNet(nn.Module):
    def __init__(
        ...
        self.layer4 = self._make_layer(block, 512, layers[3], stride=2, dilate=replace_stride_with_dilation[2], scs = scs)
        ...
        if stride != 1 or self.inplanes != planes * block.expansion:
            if scs:
                downsample = nn.Sequential(
                    scs1x1(self.inplanes, planes * block.expansion, stride),
